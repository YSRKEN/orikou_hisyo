...
　　8～14行目……各種オプションおよび特殊機能用の項目(9.以降で説明)
...
　　"自動設定II"……艦これを開いているブラウザから検索。手順としては、
　　　　　　　　「艦これを開いているブラウザ(と思われる場面を自動でアクティブにする)」
　　　　　　　　「それから5秒待つ。デバッグ時などは別の画面をアクティブにする」
　　　　　　　　「アクティブな画面について認識が行われ、以降はそちらから取得する」
　　　　　　　　となります。API的には前二つと違った手順となりますので、
　　　　　　　　艦これの画面が他の画面に覆われていても大丈夫というメリットがあります。
...
　　・編成画面……第1～第4艦隊まで対応
　　・ソート画面……編成画面で「変更」をクリックした際出る一覧が載っている画面
　　・補給画面……補給時の画面
　　・改装画面……第1～第4艦隊まで対応
　　・入渠画面……入渠ドックの画面
　　・工廠画面……工廠ドックの画面(通常時)
　　・数値設定……工廠ドックの画面(建造・開発等の暗転時)
...
　　なお、それぞれについて、最後に記録した日時がサブ画面下に表示されます。
　　また、各艦隊毎の艦娘数が、日時の横に表示されます(認識してない場合は0)。
...
7.　「【画像保存】」ボタンと、「【○○艦隊】」ボタンは、どちらも指定したフォルダ
　(通常はexeと同フォルダ)に画像を保存する機能です。この際ファイル名は、表示モードが
　現在画面の場合は現在日時+連番、それ以外は最後に記録した日時+連番で保存されます。
　連打しても上書きはされません。違いとしては次の通りです。
　　・【画像保存】
...
12.　「ソート一覧」ボタンを押すと、ソート一覧作成機能の画面が現れます。
　　サブ画面にソート画面が載っている際に「追加」ボタンを押すとリストに追加、
　　リストを選択して「削除」ボタンを押すと削除、「全削除」でリスト全削除、
　　後は出力の列数を指定して「作成」ボタンで一覧画像が作成されます。
　　なお、この時出力されるのは艦種／艦船名およびレベルのみです。
　　また、リストをクリックするとその位置に保存したソート画面画像が右に表示
　　され、「↑」「↓」ボタンで上下させることができます。

13.　以下、「オプション...」ボタンで出てくる画面の各項目について説明します。
　　・名前隠し
...
　　・大破チェック
　　　　出撃画面もしくは戦果報告の際、大破艦が存在していた場合、
　　　　出撃もしくは進撃ボタンを隠す形で白い枠が表示されます。
　　　　なお、このチェックを外すか別の画面に移行すれば解除されます。
　　・画面クロップ
...
　　　　(普通のブラウザならそうなっているはず)といった前提を元に、取得座標からの
　　　　ズレを認識し、自動で座標を取得しなおす機能です。当然再取得は重くなるので、
　　　　ズレチェックを行うタイミングは「自動設定」「自動設定II」時→1秒毎、
　　　　「手動設定」時→5秒毎に実行しています。止めたい場合は普通にチェックを
　　　　外すだけでOKです。
　　・終了セーブ
　　　　15.で後述します。
　　・フォルダ指定
　　　　スクリーンショットを保存する際に、exeと同じフォルダではないフォルダを
　　　指定したい場合には、ここにチェックを入れた後、「参照...」からフォルダを
　　　選択してください。すると、【画像保存】ボタンや【○○艦隊】ボタンや
　　　【資源画像】ボタンを押した際、ファイルがそのフォルダに保存されます。
　　・倍率～％
...
14.　「情報」ボタンを押すと、バージョン情報が表示されます。

15.　このソフトの設定、およびバッファ内の画像はソフト終了時に自動でセーブされます。
　　ただし、メイン画面の「終了セーブ」のチェックを外していた場合は、終了時の
　　セーブが行われません。
　　また、突然のシステムエラーに備えて、手動で保存することも可能です。
　　これらのデータは、ソフト起動時には自動的に読み込まれます。
　　なお、Ver.2.0からは形式が代わったので、Ver.1.9以下のセーブデータは
　　持ち越せますが逆にVer.1.9以下にセーブデータを返すことはできません(上位互換)。
...
ただし、自動設定IIでは、ブラウザの画面が隠れていても問題なく認識できます。
...
・仕様上、大破チェック機能を使用した場合、座標取得を「自動設定II」以外に
　していた場合はサブ画面にも進撃防止用の白い枠が表示されます。また、
　大破を検出した場合は、サブ画面のステータスバーにもその旨が表示されます。
...
　ほぼ同じルーチンでズレを補正します。つまり、手動設定にしているとズレを
　検出するたびに毎回マウス選択を迫られることに……。また、アルゴリズムの都合上、
　ズレを100％検知できるわけではありませんので悪しからず。

・ソート一覧で追加した画像の類は、セーブ機能ではセーブされません。
　ただし、最後に表示したソート画面はセーブ機能でセーブされます。
...
「中井技術工房」
　(中井喜博氏、http://nakai99.sakura.ne.jp/)
「あまラボ」
　(amaman氏、http://amalabo.blog35.fc2.com/)
この方々がいなければこのソフトウェアは生まれませんでした。深く感謝いたします。

[更新履歴]
＜Ver.2.0(2015/05/02)＞
・オプション画面を新設。これにより画面サイズがコンパクトになった。
・大破チェックを出撃時にも行うようにした。
...
＜Ver.1.9(2015/04/30)＞
・編成まとめ画像作成時に、艦隊毎の艦娘数を誤爆する不具合を修正。
　また、各艦隊の艦娘数をいつでも確認できるようにした。

＜Ver.1.8(2015/04/28)＞
・サブ画面に、各場面毎の最終更新日時を表示するようにした(セーブもされる)。
・シーンに「戦闘画面」を新設。昼戦画面と夜戦画面で最後に表示したものを記録する。
　……まあこれVer.1.3まで付いてた「戦闘画面」を再現しただけなんですけどね！
・大破チェックの仕様を変更して、大破発生時は進撃ボタンがありそうな部分を
　別画面で隠すようにした。チェックを外すか別のシーンに移行すれば消えます。
・自動ズレ補正機能のバグを修正。真横(左)に動く場合に検出してなかった……。
・大破チェックの仕様変更に応じて、自動設定IIでの取得方法を微妙に変更。
　具体的にはクライアント領域だけではなく非クライアント領域を含めてキャプチャ
　するようにした。これだけでAPIがガラっと変わるのがWinAPIの恐ろしさよ……。
　またこれに伴い、自動設定IIでもセーブされた座標がちゃんと効くようになった。
　(今まではクライアント領域基準の座標だったので「自動設定」「手動設定」とかに
　　使いまわせなかった)

＜Ver.1.7(2015/04/27)＞
・「自動設定II」を新設。艦これの画面が他の画面に隠されていても取得できるようになった。

＜Ver.1.6(2015/04/26)＞
・自動ズレ補正機能にて、座標取得を手動設定にしていた場合は再取得の間隔を1秒
　ではなく5秒にした。そうしないと、「連続でマウス選択を迫られてチェック外せない」
　といった事故が頻発するので……。
・ソートまとめ画像作成機能を追加した。
